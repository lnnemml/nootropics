...
| Payments | **Manual confirmation flow (Phase 1)** — no processor wired into code. Real processor TBD: high-risk offshore merchant account, currently being researched by Anton in parallel | Stripe and equivalent mainstream processors reject/terminate accounts for this product category (nootropics/bromantane reads as high-risk). Building a real-time gateway integration now would be wasted work against an unknown future processor's API. See [`../decisions/0005-manual-payment-flow.md`](../decisions/0005-manual-payment-flow.md) | Stripe (rejected — not available for this product category, confirmed by Anton) |
| Email (transactional) | **Resend** | Order-received confirmation to customer + internal new-order notification to ops, simple API, good Next.js DX | SES (more setup overhead for this volume), Postmark (fine alternative, no strong reason over Resend) |
